...
# Home, IP Lookup, and Stellar to Google Form in one window.

Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
Add-Type -AssemblyName System.Web

# This function runs ONLY in the separate PowerShell worker below. Loading UI
# Automation in the GUI process can change the scale of the SOC Tools window.
# Google Forms does not expose its entry IDs in a restricted respondent link,
# so the worker finds each question by its accessible label and fills it after
# the signed-in form opens in Chrome.
function Invoke-GoogleFormPrefillWorker {
    param(
    ...
    $googleFormShell = New-Object -ComObject WScript.Shell
    $googleFormDeadline = [DateTime]::UtcNow.AddSeconds($TimeoutSeconds)

    # Google Forms may append a required-field asterisk or "Your answer" to
    # the accessible name. Normalize those additions before matching labels.
    $normalizeGoogleFormName = {
        param([string]$Name)
        ...
        $normalizedName = $Name -replace `
            '(?i)\s*(?:Your answer|Long-answer text)\s*$', ''
        $normalizedName = $normalizedName -replace `
            '(?i)\s*(?:\*|Required)\s*$', ''

        return $normalizedName.Trim().TrimEnd([char]':')
    }

    $getGoogleFormElements = {
    ...
    $findGoogleFormControl = {
        param(
            [System.Windows.Automation.AutomationElementCollection]$Elements,
            [object[]]$Aliases,
            [bool]$RequireKeyboardFocus = $true,
            [bool]$AllowPrefix = $true
        )

        $normalizedAliases = @(
            $Aliases |
            ForEach-Object { & $normalizeGoogleFormName ([string]$_) } |
        ...
        for ($elementIndex = 0; $elementIndex -lt $Elements.Count; $elementIndex++) {
            ...
            try {
                ...
                if ($RequireKeyboardFocus -and -not $element.Current.IsKeyboardFocusable) {
                    continue
                }

                # Chrome sometimes exposes only "Your answer" as the control
                # name and keeps the question title in LabeledBy or HelpText.
                $candidateNames = @($element.Current.Name)

                try {
                    $labeledByElement = $element.Current.LabeledBy
                    if ($null -ne $labeledByElement) {
                        $candidateNames += $labeledByElement.Current.Name
                    }
                }
                catch {
                    # Some Chrome nodes do not expose LabeledBy.
                }

                try {
                    if (-not [string]::IsNullOrWhiteSpace($element.Current.HelpText)) {
                        $candidateNames += $element.Current.HelpText
                    }
                }
                catch {
                ...
                foreach ($candidateName in $candidateNames) {
                    $normalizedName = & $normalizeGoogleFormName ([string]$candidateName)
                    foreach ($alias in $normalizedAliases) {
                        if (
                            $normalizedName -ieq $alias -or
                            ($AllowPrefix -and $normalizedName -ilike "$alias *")
                        ) {
                            return $element
                        }
                    }
                ...
                # Chrome can replace accessibility nodes as the form renders.
            }
        }
    ...
    # Google Forms often separates a question's visible label from its generic
    # "Your answer" edit control. When accessibility does not connect them,
    # locate the label and choose the nearest editable control beneath it.
    $findGoogleFormEditNearLabel = {
        param(
            [System.Windows.Automation.AutomationElementCollection]$Elements,
            [object[]]$Aliases
        )

        $labelElement = & $findGoogleFormControl `
            -Elements $Elements `
            -Aliases $Aliases `
            -RequireKeyboardFocus $false `
            -AllowPrefix $true

        if ($null -eq $labelElement) {
            return $null
        }
        ...
        if ($labelRectangle.IsEmpty) {
            return $null
        }

        $bestEditControl = $null
        $bestEditScore = [double]::PositiveInfinity

        for ($editIndex = 0; $editIndex -lt $Elements.Count; $editIndex++) {
            $editCandidate = $Elements.Item($editIndex)

            try {
                if (
                    -not $editCandidate.Current.IsEnabled -or
                    -not $editCandidate.Current.IsKeyboardFocusable -or
                    $editCandidate.Current.ControlType -ne [System.Windows.Automation.ControlType]::Edit
                ) {
                    continue
                }

                $editCandidateName = [string]$editCandidate.Current.Name
                if (
                    $editCandidateName -match '(?i)Address and search bar|Search Google|Find in page'
                ) {
                    continue
                }

                $editRectangle = $editCandidate.Current.BoundingRectangle
                if ($editRectangle.IsEmpty -or $editRectangle.Width -lt 40) {
                    continue
                }

                # Answer boxes are normally below, or vertically overlapping,
                # their question label. Reject controls above or far away.
                $verticalGap = $editRectangle.Top - $labelRectangle.Bottom
                if ($verticalGap -lt -25 -or $verticalGap -gt 350) {
                    continue
                }

                $labelCenterX = $labelRectangle.Left + ($labelRectangle.Width / 2)
                $editCenterX = $editRectangle.Left + ($editRectangle.Width / 2)
                $horizontalGap = [Math]::Abs($editCenterX - $labelCenterX)
                $editScore = ([Math]::Abs($verticalGap) * 10) + $horizontalGap

                if ($editScore -lt $bestEditScore) {
                    $bestEditScore = $editScore
                    $bestEditControl = $editCandidate
                }
            }
            catch {
                # Continue when Chrome replaces an accessibility node.
            }
        }

        return $bestEditControl
    }
    ...
    $setGoogleFormText = {
        ...
        try {
            ...
            if (
                ...
                -not $valuePatternObject.Current.IsReadOnly
            ) {
                $valuePatternObject.SetValue($Value)
                return $true
            }
        }
        catch {
            # Fall back to the clipboard for Chrome controls without ValuePattern.
        }
    ...
    $selectGoogleFormOption = {
        param(
            [System.Windows.Automation.AutomationElement]$Root,
            [object[]]$QuestionAliases,
            [string]$OptionValue
        )
        ...
        for ($selectionAttempt = 0; $selectionAttempt -lt 2; $selectionAttempt++) {
            $currentElements = & $getGoogleFormElements $Root
            $optionElement = & $findGoogleFormControl `
                -Elements $currentElements `
                -Aliases @($OptionValue) `
                -RequireKeyboardFocus $true `
                -AllowPrefix $false

            if ($null -ne $optionElement) {
                try {
                    $togglePatternObject = $null
                    if ($optionElement.TryGetCurrentPattern(
                        [System.Windows.Automation.TogglePattern]::Pattern,
                        [ref]$togglePatternObject
                    )) {
                        if (
                            $togglePatternObject.Current.ToggleState -eq
                            [System.Windows.Automation.ToggleState]::Off
                        ) {
                            $togglePatternObject.Toggle()
                        }
                        return $true
                    }
                    ...
                    # If the option was replaced, open its question and retry.
                }
            }

            $questionElement = & $findGoogleFormControl `
                -Elements $currentElements `
                -Aliases $QuestionAliases `
                -RequireKeyboardFocus $true

            if ($null -eq $questionElement) {
                return $false
            }

            try {
                if (
                    $questionElement.Current.ControlType -eq
                    [System.Windows.Automation.ControlType]::Edit
                ) {
                    return (& $setGoogleFormText $questionElement $OptionValue)
                }

                $expandPatternObject = $null
                if ($questionElement.TryGetCurrentPattern(
    ...
    while ([DateTime]::UtcNow -lt $googleFormDeadline) {
        $googleFormActivated = $false
        $googleFormChromeWindow = $null
        $googleFormSearchRoot = $null

        # A newly opened form window should already be in front. Use its
        # focused accessibility element first so another Chrome window cannot
        # be selected accidentally.
        try {
            $googleFormFocusedElement = [System.Windows.Automation.AutomationElement]::FocusedElement

            if ($null -ne $googleFormFocusedElement) {
                $googleFormFocusedProcess = Get-Process `
                    -Id $googleFormFocusedElement.Current.ProcessId `
                    -ErrorAction Stop

                if ($googleFormFocusedProcess.ProcessName -ieq "chrome") {
                    $googleFormSearchRoot = $googleFormFocusedElement
                    $googleFormTreeWalker = [System.Windows.Automation.TreeWalker]::RawViewWalker

                    while ($null -ne $googleFormSearchRoot) {
                        $googleFormParent = $googleFormTreeWalker.GetParent($googleFormSearchRoot)
                        if (
                            $null -eq $googleFormParent -or
                            $googleFormParent.Current.ProcessId -eq 0
                        ) {
                            break
                        }
                        $googleFormSearchRoot = $googleFormParent
                    }

                    $googleFormActivated = ($null -ne $googleFormSearchRoot)
                }
            }
        }
        catch {
        ...
        # Chrome's title starts with the form name, so AppActivate("Google
        # Forms") normally misses it. Activate the real Chrome window by PID.
        if (-not $googleFormActivated) {
            $googleFormChromeWindows = @(
                Get-Process -Name "chrome" -ErrorAction SilentlyContinue |
                Where-Object { $_.MainWindowHandle -ne [IntPtr]::Zero }
            )

            $googleFormPreferredWindows = @(
                $googleFormChromeWindows |
                Where-Object { $_.MainWindowTitle -match '(?i)Google Forms' } |
                Sort-Object -Property Id -Descending
            )
            $googleFormOtherWindows = @(
                $googleFormChromeWindows |
                Where-Object { $_.MainWindowTitle -notmatch '(?i)Google Forms' } |
                Sort-Object -Property Id -Descending
            )

            foreach ($googleFormWindowCandidate in @(
                $googleFormPreferredWindows + $googleFormOtherWindows
            )) {
                try {
                    if ($googleFormShell.AppActivate([int]$googleFormWindowCandidate.Id)) {
                        $googleFormActivated = $true
                        $googleFormChromeWindow = $googleFormWindowCandidate
                        break
                    }
                }
                catch {
                    # Try the next visible Chrome window.
                }
            }
        }

        if ($googleFormActivated) {
            Start-Sleep -Milliseconds 400

            try {
                if (
                    $null -eq $googleFormSearchRoot -and
                    $null -ne $googleFormChromeWindow
                ) {
                    $googleFormSearchRoot = [System.Windows.Automation.AutomationElement]::FromHandle(
                        $googleFormChromeWindow.MainWindowHandle
                    )
                }

                if ($null -eq $googleFormSearchRoot) {
                    throw "Chrome accessibility root is unavailable."
                }

                # Try every field independently. A missing or renamed question
                # must not prevent later questions from being filled.
                $googleFormFilledCount = 0
                foreach ($textField in $TextFields) {
                    $fieldValue = [string]$textField.Value
                    if ([string]::IsNullOrWhiteSpace($fieldValue)) {
                        continue
                    }

                    $googleFormElements = & $getGoogleFormElements $googleFormSearchRoot
                    $textControl = & $findGoogleFormControl `
                        -Elements $googleFormElements `
                        -Aliases @($textField.Aliases) `
                        -RequireKeyboardFocus $true

                    if ($null -eq $textControl) {
                        $textControl = & $findGoogleFormEditNearLabel `
                            -Elements $googleFormElements `
                            -Aliases @($textField.Aliases)
                    }

                    if (& $setGoogleFormText $textControl $fieldValue) {
                        $googleFormFilledCount++
                    }
                }

                foreach ($choiceField in $ChoiceFields) {
                    $choiceFilled = & $selectGoogleFormOption `
                        -Root $googleFormSearchRoot `
                        -QuestionAliases @($choiceField.Aliases) `
                        -OptionValue ([string]$choiceField.Value)
                    if ($choiceFilled) {
                        $googleFormFilledCount++
                    }
                }

                foreach ($selectedAnalyst in $SelectedAnalysts) {
                    $analystFilled = & $selectGoogleFormOption `
                        -Root $googleFormSearchRoot `
                        -QuestionAliases @("Analyst", "Analysts", "Analysts on Shift") `
                        -OptionValue ([string]$selectedAnalyst)
                    if ($analystFilled) {
                        $googleFormFilledCount++
                    }
                }

                # If Chrome opened but the form has not rendered yet, retry.
                # Once any field is filled, keep all successful partial results.
                if ($googleFormFilledCount -eq 0) {
                    throw "No matching Google Form fields are available yet."
                }

                # Leave the cursor at the end of Issue for the analyst's review.
                $googleFormElements = & $getGoogleFormElements $googleFormSearchRoot
                $issueControl = & $findGoogleFormControl `
                    -Elements $googleFormElements `
                    -Aliases @("Issue", "Issue:") `
                    -RequireKeyboardFocus $true
                if ($null -ne $issueControl) {
                    $issueControl.SetFocus()
                    [System.Windows.Forms.SendKeys]::SendWait("^{END}")
                }

                if (-not [string]::IsNullOrWhiteSpace($ClipboardFallback)) {
                    [System.Windows.Forms.Clipboard]::SetText($ClipboardFallback)
                }
                return $true
            }
            catch {
                # Keep retrying while the authenticated form and its fields load.
            }
        }
    ...
    if (-not [string]::IsNullOrWhiteSpace($ClipboardFallback)) {
        try {
            ...
            # The caller will display the automatic-fill failure message.
        }
    }

    # If nothing matched, save the control names Google exposed. This keeps a
    # future failure diagnosable without recording any values the script tried
    # to enter into the form.
    if (
        -not [string]::IsNullOrWhiteSpace($DebugFilePath) -and
        $null -ne $googleFormSearchRoot
    ) {
        try {
            $debugElements = & $getGoogleFormElements $googleFormSearchRoot
            $debugLines = @(
                "Google Forms accessibility controls"
                "Generated: $([DateTime]::Now.ToString('yyyy-MM-dd HH:mm:ss'))"
                ""
            )

            for ($debugIndex = 0; $debugIndex -lt $debugElements.Count; $debugIndex++) {
                $debugElement = $debugElements.Item($debugIndex)
                try {
                    $debugControlType = $debugElement.Current.ControlType.ProgrammaticName
                    if (
                        $debugControlType -notmatch 'Edit|Text|Document|ComboBox|ListItem|RadioButton|CheckBox'
                    ) {
                        continue
                    }

                    $debugLabel = ""
                    if ($null -ne $debugElement.Current.LabeledBy) {
                        $debugLabel = [string]$debugElement.Current.LabeledBy.Current.Name
                    }

                    $debugLines += (
                        "Type={0} | Name={1} | Label={2} | Help={3} | Focusable={4}" -f
                        $debugControlType,
                        [string]$debugElement.Current.Name,
                        $debugLabel,
                        [string]$debugElement.Current.HelpText,
                        [string]$debugElement.Current.IsKeyboardFocusable
                    )
                }
                catch {
                    # Ignore controls that disappear while the dump is built.
                }
            }

            $debugLines | Set-Content -Path $DebugFilePath -Encoding UTF8
        }
        catch {
            # Debug output must never interrupt the main SOC tool.
        }
    }
    return $false
}
...
# SOC Tools window. Long text and Unicode travel through standard input rather
# than command-line arguments or temporary files.
function Set-GoogleFormFields {
    param(
        [Parameter(Mandatory = $true)]
        [object[]]$TextFields,
    ...
    $googleFormWorkerProcess = $null
    $googleFormWorkerStarted = $false

    try {
        $googleFormWorkerRequest = @{
            WorkerScript = ${function:Invoke-GoogleFormPrefillWorker}.ToString()
        ...
        $googleFormRequestXml = [System.Management.Automation.PSSerializer]::Serialize(
            $googleFormWorkerRequest
        )
        $googleFormRequestBase64 = [Convert]::ToBase64String(
            [System.Text.Encoding]::Unicode.GetBytes($googleFormRequestXml)
        )

        $googleFormWorkerBootstrap = @'
...
try {
    ...
    $filled = & $worker `
        ...
        -ChoiceFields $request.ChoiceFields `
        -SelectedAnalysts $request.SelectedAnalysts `
        -ClipboardFallback $request.ClipboardFallback `
        -DebugFilePath $request.DebugFilePath `
        -TimeoutSeconds $request.TimeoutSeconds
    if ($filled -eq $true) { exit 0 }
    exit 1
}
catch {
    exit 1
...
'@
        $googleFormEncodedBootstrap = [Convert]::ToBase64String(
            [System.Text.Encoding]::Unicode.GetBytes($googleFormWorkerBootstrap)
        )

        $googleFormPowerShellPath = Join-Path -Path ([Environment]::GetFolderPath('System')) `
            -ChildPath 'WindowsPowerShell\v1.0\powershell.exe'
        $googleFormStartInfo = New-Object System.Diagnostics.ProcessStartInfo
        $googleFormStartInfo.FileName = $googleFormPowerShellPath
        $googleFormStartInfo.Arguments = '-NoLogo -NoProfile -NonInteractive -STA -WindowStyle Hidden -EncodedCommand ' + $googleFormEncodedBootstrap
        $googleFormStartInfo.UseShellExecute = $false
        $googleFormStartInfo.CreateNoWindow = $true
        $googleFormStartInfo.WindowStyle = [System.Diagnostics.ProcessWindowStyle]::Hidden
        $googleFormStartInfo.RedirectStandardInput = $true

        $googleFormWorkerProcess = New-Object System.Diagnostics.Process
        $googleFormWorkerProcess.StartInfo = $googleFormStartInfo
        $googleFormWorkerStarted = $googleFormWorkerProcess.Start()
        if (-not $googleFormWorkerStarted) {
            return $false
        }

        $googleFormWorkerProcess.StandardInput.Write($googleFormRequestBase64)
        $googleFormWorkerProcess.StandardInput.Close()

        $googleFormWorkerWaitMs = ($TimeoutSeconds + 15) * 1000
        if (-not $googleFormWorkerProcess.WaitForExit($googleFormWorkerWaitMs)) {
            return $false
        }

        return ($googleFormWorkerProcess.ExitCode -eq 0)
    }
    catch {
        return $false
    }
    finally {
        if ($null -ne $googleFormWorkerProcess) {
            try {
                if ($googleFormWorkerStarted -and -not $googleFormWorkerProcess.HasExited) {
                    $googleFormWorkerProcess.Kill()
                    [void]$googleFormWorkerProcess.WaitForExit(2000)
                }
            }
            catch {
                ...
                $googleFormWorkerProcess.Dispose()
            }
        }
    }
}

# Find the open BlackSwan/Stellar tab before building the Google Form data.
...
# spaces, and line breaks are preserved for readability in the Google Form.
...
# paragraph can be added to the Google Form without switching browser tabs.
...
# Read a short value from Stellar's copied page text. Different Chrome and
# Stellar layouts place the value after a colon, after a tab, or on the next
# nonblank line, so support all three formats.
function Get-StellarLabeledValue {
    param(
        [string]$Text,
        [string[]]$Labels
    )

    if ([string]::IsNullOrWhiteSpace($Text) -or $Labels.Count -eq 0) {
        return ""
    }

    $stellarLabeledLines = @($Text -split "`r?`n")
    $stellarStopLabels = @(
        "Who", "What", "When", "Where", "Severity", "Tenant", "Tenant Name",
        "Case Score Breakdown", "Associated Alerts", "Overview", "Alert Details",
        "Kill Chain Stage", "MITRE Tactic", "MITRE Technique", "Run Analysis", "JSON",
        "Stellar #", "Stellar Number", "Case Number", "Issue", "Alert Name",
        "URL", "Source Host", "Source IP", "SIP", "Destination Host",
        "Destination IP", "DIP", "Description", "Supporting Notes", "Detection Type"
    )

    for ($stellarLineIndex = 0; $stellarLineIndex -lt $stellarLabeledLines.Count; $stellarLineIndex++) {
        $stellarCurrentLine = $stellarLabeledLines[$stellarLineIndex].Trim()

        foreach ($stellarLabelName in $Labels) {
            $stellarEscapedLabel = [regex]::Escape($stellarLabelName)
            $stellarInlineMatch = [regex]::Match(
                $stellarCurrentLine,
                "(?i)^$stellarEscapedLabel(?:\s*:\s*|\t+|\s{2,})(?<Value>.+?)\s*$"
            )

            if ($stellarInlineMatch.Success) {
                return ($stellarInlineMatch.Groups["Value"].Value -replace '\s+', ' ').Trim()
            }

            if ($stellarCurrentLine -notmatch "(?i)^$stellarEscapedLabel\s*:?\s*$") {
                continue
            }

            for (
                $stellarValueIndex = $stellarLineIndex + 1;
                $stellarValueIndex -lt $stellarLabeledLines.Count -and
                $stellarValueIndex -le ($stellarLineIndex + 4);
                $stellarValueIndex++
            ) {
                $stellarCandidateValue = $stellarLabeledLines[$stellarValueIndex].Trim()

                if ([string]::IsNullOrWhiteSpace($stellarCandidateValue)) {
                    continue
                }

                $stellarCandidateIsLabel = $false
                foreach ($stellarStopLabel in $stellarStopLabels) {
                    if ($stellarCandidateValue -match "(?i)^$([regex]::Escape($stellarStopLabel))\s*:?\s*$") {
                        $stellarCandidateIsLabel = $true
                        break
                    }
                }

                if ($stellarCandidateIsLabel) {
                    break
                }

                return ($stellarCandidateValue -replace '\s+', ' ').Trim()
            }
        }
    }
...
# Excel checksheet stored in SharePoint. The Google Form used by the
# Stellar tab remains configured separately below.
...
$stellarTab.Text = "Stellar Parser"
$stellarTab.BackColor = [System.Drawing.Color]::FromArgb(32, 34, 37)

$formCopyTab = New-Object System.Windows.Forms.TabPage
$formCopyTab.Text = "Form Copy Fields"
$formCopyTab.BackColor = [System.Drawing.Color]::FromArgb(245, 248, 252)
...
[void]$tabControl.TabPages.Add($formCopyTab)
...
$stellarGoogleFormUrl = "https://docs.google.com/forms/d/10meqp_NTemVFcPXcikt3A01p2ucTFW_IAthY67SGxkk/viewform"
...
# ============================================================
# FORM COPY FIELDS TAB
# ============================================================

$formCopyValues = [ordered]@{}
$formCopyDropdownFields = @(
    "Tenant Name*"
    "Analyst*"
    "Stellar Severity*"
    "Kill Chain Stage*"
...
$formCopyInstructions = New-Object System.Windows.Forms.Label
$formCopyInstructions.Text = "Dropdown answers are shown in the table. Click a text-field row to copy its value."
$formCopyInstructions.Location = New-Object System.Drawing.Point(20, 18)
$formCopyInstructions.Size = New-Object System.Drawing.Size(500, 42)
$formCopyInstructions.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
$formCopyInstructions.ForeColor = [System.Drawing.Color]::FromArgb(35, 45, 55)
$formCopyInstructions.Anchor = "Top, Left, Right"
$formCopyTab.Controls.Add($formCopyInstructions)

$formCopyOpenFormButton = New-Object System.Windows.Forms.Button
$formCopyOpenFormButton.Text = "Open Google Form"
$formCopyOpenFormButton.Location = New-Object System.Drawing.Point(540, 15)
$formCopyOpenFormButton.Size = New-Object System.Drawing.Size(140, 30)
$formCopyOpenFormButton.FlatStyle = "Flat"
$formCopyOpenFormButton.FlatAppearance.BorderSize = 0
$formCopyOpenFormButton.BackColor = [System.Drawing.Color]::FromArgb(0, 120, 215)
$formCopyOpenFormButton.ForeColor = [System.Drawing.Color]::White
$formCopyOpenFormButton.Cursor = [System.Windows.Forms.Cursors]::Hand
$formCopyOpenFormButton.Anchor = "Top, Right"
$formCopyTab.Controls.Add($formCopyOpenFormButton)

$formCopyAllButton = New-Object System.Windows.Forms.Button
$formCopyAllButton.Text = "Copy Text Fields"
$formCopyAllButton.Location = New-Object System.Drawing.Point(540, 50)
$formCopyAllButton.Size = New-Object System.Drawing.Size(140, 30)
$formCopyAllButton.FlatStyle = "Flat"
$formCopyAllButton.FlatAppearance.BorderSize = 0
$formCopyAllButton.BackColor = [System.Drawing.Color]::FromArgb(75, 90, 105)
$formCopyAllButton.ForeColor = [System.Drawing.Color]::White
$formCopyAllButton.Cursor = [System.Windows.Forms.Cursors]::Hand
$formCopyAllButton.Anchor = "Top, Right"
$formCopyTab.Controls.Add($formCopyAllButton)

$formCopyFieldLabel = New-Object System.Windows.Forms.Label
$formCopyFieldLabel.Text = "Google Form fields in submission order"
$formCopyFieldLabel.Location = New-Object System.Drawing.Point(20, 70)
$formCopyFieldLabel.Size = New-Object System.Drawing.Size(350, 22)
$formCopyFieldLabel.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
$formCopyFieldLabel.ForeColor = [System.Drawing.Color]::FromArgb(35, 45, 55)
$formCopyTab.Controls.Add($formCopyFieldLabel)

$formCopyPreviewLabel = New-Object System.Windows.Forms.Label
$formCopyPreviewLabel.Text = "Full selected value"
$formCopyPreviewLabel.Location = New-Object System.Drawing.Point(20, 470)
$formCopyPreviewLabel.Size = New-Object System.Drawing.Size(200, 22)
$formCopyPreviewLabel.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
$formCopyPreviewLabel.ForeColor = [System.Drawing.Color]::FromArgb(35, 45, 55)
$formCopyTab.Controls.Add($formCopyPreviewLabel)

$formCopyGrid = New-Object System.Windows.Forms.DataGridView
$formCopyGrid.Location = New-Object System.Drawing.Point(20, 95)
$formCopyGrid.Size = New-Object System.Drawing.Size(660, 365)
$formCopyGrid.Font = New-Object System.Drawing.Font("Segoe UI", 9)
$formCopyGrid.BackgroundColor = [System.Drawing.Color]::White
$formCopyGrid.ForeColor = [System.Drawing.Color]::FromArgb(35, 45, 55)
$formCopyGrid.BorderStyle = "FixedSingle"
$formCopyGrid.ReadOnly = $true
$formCopyGrid.AllowUserToAddRows = $false
$formCopyGrid.AllowUserToDeleteRows = $false
$formCopyGrid.AllowUserToResizeRows = $false
$formCopyGrid.MultiSelect = $false
$formCopyGrid.RowHeadersVisible = $false
$formCopyGrid.SelectionMode = "FullRowSelect"
$formCopyGrid.AutoSizeRowsMode = "None"
$formCopyGrid.RowTemplate.Height = 24
$formCopyGrid.Anchor = "Top, Left, Right"

[void]$formCopyGrid.Columns.Add("FormField", "Form field")
[void]$formCopyGrid.Columns.Add("Answer", "Answer")
[void]$formCopyGrid.Columns.Add("Action", "Action")
$formCopyGrid.Columns["FormField"].Width = 190
$formCopyGrid.Columns["Answer"].AutoSizeMode = "Fill"
$formCopyGrid.Columns["Action"].Width = 105
$formCopyGrid.Columns["FormField"].SortMode = "NotSortable"
$formCopyGrid.Columns["Answer"].SortMode = "NotSortable"
$formCopyGrid.Columns["Action"].SortMode = "NotSortable"
$formCopyTab.Controls.Add($formCopyGrid)

$formCopyPreviewBox = New-Object System.Windows.Forms.TextBox
$formCopyPreviewBox.Location = New-Object System.Drawing.Point(20, 495)
$formCopyPreviewBox.Size = New-Object System.Drawing.Size(660, 65)
$formCopyPreviewBox.Multiline = $true
$formCopyPreviewBox.ReadOnly = $true
$formCopyPreviewBox.WordWrap = $true
$formCopyPreviewBox.ScrollBars = "Both"
$formCopyPreviewBox.BackColor = [System.Drawing.Color]::White
$formCopyPreviewBox.ForeColor = [System.Drawing.Color]::FromArgb(35, 35, 35)
$formCopyPreviewBox.BorderStyle = "FixedSingle"
$formCopyPreviewBox.Font = New-Object System.Drawing.Font("Consolas", 10)
$formCopyPreviewBox.Anchor = "Top, Bottom, Left, Right"
$formCopyTab.Controls.Add($formCopyPreviewBox)

$formCopyStatusLabel = New-Object System.Windows.Forms.Label
$formCopyStatusLabel.Text = "Parse a Stellar alert to build the copy list."
$formCopyStatusLabel.Location = New-Object System.Drawing.Point(20, 575)
$formCopyStatusLabel.Size = New-Object System.Drawing.Size(660, 28)
$formCopyStatusLabel.ForeColor = [System.Drawing.Color]::FromArgb(55, 95, 135)
$formCopyStatusLabel.Anchor = "Bottom, Left, Right"
$formCopyTab.Controls.Add($formCopyStatusLabel)

$formCopyGrid.Add_CellClick({
    if ($_.RowIndex -lt 0) {
        return
    }

    $formCopySelectedRow = $formCopyGrid.Rows[$_.RowIndex]
    $formCopySelectedField = [string]$formCopySelectedRow.Cells["FormField"].Value
    $formCopySelectedValue = [string]$formCopyValues[$formCopySelectedField]
    $formCopyPreviewBox.Text = $formCopySelectedValue

    if ($formCopyDropdownFields -contains $formCopySelectedField) {
        if ([string]::IsNullOrWhiteSpace($formCopySelectedValue)) {
            $formCopyStatusLabel.Text = "Dropdown answer not detected: $formCopySelectedField"
        }
        else {
            $formCopyStatusLabel.Text = "Dropdown selection: $formCopySelectedField  $formCopySelectedValue"
        }
        return
    }

    if ([string]::IsNullOrWhiteSpace($formCopySelectedValue)) {
        $formCopyStatusLabel.Text = "No value detected for: $formCopySelectedField"
        return
    }

    [System.Windows.Forms.Clipboard]::SetText($formCopySelectedValue)
    $formCopyStatusLabel.Text = "Copied: $formCopySelectedField"
})

$formCopyAllButton.Add_Click({
    if ($formCopyValues.Count -eq 0) {
        $formCopyStatusLabel.Text = "Parse a Stellar alert before copying fields."
        return
    }

    $formCopyAllText = @(
        foreach ($formCopyFieldName in $formCopyValues.Keys) {
            if (
                $formCopyDropdownFields -notcontains $formCopyFieldName -and
                -not [string]::IsNullOrWhiteSpace([string]$formCopyValues[$formCopyFieldName])
            ) {
                "${formCopyFieldName}:`r`n$($formCopyValues[$formCopyFieldName])"
            }
        }
    ) -join "`r`n`r`n"

    if ([string]::IsNullOrWhiteSpace($formCopyAllText)) {
        $formCopyStatusLabel.Text = "No text-field values were detected."
        return
    }

    [System.Windows.Forms.Clipboard]::SetText($formCopyAllText)
    $formCopyStatusLabel.Text = "Copied all detected text fields."
})

$formCopyOpenFormButton.Add_Click({
    Start-Process "chrome.exe" -ArgumentList @("--new-window", "--start-maximized", $stellarGoogleFormUrl)
})
...
# first using Fill Google Form on the Stellar tab.
...
# Rebuild the two lists only when Fill Google Form is pressed. Exact duplicate
...
$caseIpCheckJsonButton.Add_Click({
    ...
    # Reuse the same IP extraction used by Fill Google Form, then immediately run
...
$stellarAnalystLabel.Text = "Analyst:"
...
$stellarAnalystComboBox = New-Object System.Windows.Forms.ComboBox
$stellarAnalystComboBox.Location = New-Object System.Drawing.Point(20, 445)
$stellarAnalystComboBox.Size = New-Object System.Drawing.Size(300, 30)
$stellarAnalystComboBox.DropDownStyle = "DropDownList"
$stellarAnalystComboBox.BackColor = [System.Drawing.Color]::FromArgb(38, 42, 46)
$stellarAnalystComboBox.ForeColor = [System.Drawing.Color]::FromArgb(225, 225, 225)
$stellarAnalystComboBox.FlatStyle = "Flat"
$stellarTab.Controls.Add($stellarAnalystComboBox)

foreach ($stellarAnalyst in $stellarAnalysts) {
    [void]$stellarAnalystComboBox.Items.Add($stellarAnalyst)
}

if ($stellarAnalystComboBox.Items.Count -gt 0) {
    $stellarAnalystComboBox.SelectedIndex = 0
}
...
$stellarRunCaseIpLookupCheckBox.Location = New-Object System.Drawing.Point(340, 448)
$stellarRunCaseIpLookupCheckBox.Size = New-Object System.Drawing.Size(260, 25)
...
# ==========================================
# PREPARE FORM FIELDS BUTTON
# ==========================================

$stellarButton = New-Object System.Windows.Forms.Button
$stellarButton.Text = "Prepare Form Fields"
...
$stellarButton.Add_Click({
    ...
    if ($stellarRunCaseIpLookupCheckBox.Checked) {
        ...
        $caseIpStatusLabel.Text = "Case IP lookup skipped (unchecked on Parser tab)."
    }
    ...
    if ($stellarText -match '(?i)Sophos|\bEDR\b|endpoint detection') {
        $stellarDetectionType = "EDR Alert"
    }
    elseif ($stellarText -match '(?i)phish') {
        $stellarDetectionType = "Phishing"
    }
    elseif ($stellarText -match '(?i)firewall') {
        $stellarDetectionType = "Firewall"
    }
    elseif ($stellarText -match '(?i)\bIDS\b|\bIPS\b|intrusion detection|intrusion prevention') {
        $stellarDetectionType = "IDS/IPS"
    }
    elseif ($stellarText -match '(?i)user[ -]?reported') {
        $stellarDetectionType = "User Reported"
    }
    elseif ($stellarText -match '(?i)threat intel|indicator of compromise|\bIOC\b.*match') {
        $stellarDetectionType = "Threat Intel Match"
    }
    elseif ($stellarText -match '(?i)XDR|SIEM|correlation') {
    ...
    # Prefer a BlackSwan URL already present in the copied page text. If Chrome
    # did not include it, locate the open case tab through the address bar.
    $stellarUrlValue = ""
    $stellarPastedUrlMatch = [regex]::Match(
        $stellarText,
        '(?i)https://(?:[^\s/]+\.)?blackswan\.stellarcyber\.cloud/[^\s]+'
    )

    if ($stellarPastedUrlMatch.Success) {
        $stellarUrlValue = $stellarPastedUrlMatch.Value.TrimEnd([char[]]".,)]")
    }
    else {
        $stellarUrlValue = Get-BlackSwanChromeUrl
    }

    # The Google Form only needs the case URL. Remove the event query string.
    if (-not [string]::IsNullOrWhiteSpace($stellarUrlValue)) {
        $stellarUrlValue = ($stellarUrlValue -split '\?', 2)[0]
    }
    ...
    if (-not $stellarStellar) {
        $stellarStellar = Get-StellarLabeledValue `
            -Text $stellarText `
            -Labels @("Stellar #", "Stellar Number", "Case Number")
    }

    if (
        -not $stellarStellar -and
        $stellarJsonText -match '(?i)"(?:case_number|case_no|stellar_number)"\s*:\s*"?(\d+)"?'
    ) {
        $stellarStellar = $matches[1]
    }
    ...
    if ($stellarIssueMatch.Success) {
        ...
        # Chrome can insert line breaks when an alert title wraps. The form's
    ...
    if (-not $stellarIssue) {
        $stellarIssue = Get-StellarLabeledValue `
            -Text $stellarText `
            -Labels @("Issue", "Alert Name", "Display Name")
    }
    ...
    $stellarWho = Get-StellarLabeledValue -Text $stellarText -Labels @("Who")

    if (-not $stellarWho) {
        $stellarWho = $stellarJsonUser
    }
    ...
    $stellarWhat = Get-StellarLabeledValue -Text $stellarText -Labels @("What")

    if (-not $stellarWhat) {
        $stellarWhat = $stellarIssue
    }
    ...
    $stellarWhen = Get-StellarLabeledValue -Text $stellarText -Labels @("When")

    if (
        -not $stellarWhen -and
        $stellarJsonText -match '(?i)"(?:@timestamp|event_time|timestamp|write_time)"\s*:\s*"([^"]+)"'
    ) {
    ...
    $stellarWhere = Get-StellarLabeledValue -Text $stellarText -Labels @("Where")
    ...
    $stellarSeverity = Get-StellarLabeledValue -Text $stellarText -Labels @("Severity")

    if (
        -not $stellarSeverity -and
        $stellarJsonText -match '(?i)"(?:severity|event_severity|xdr_event\.severity)"\s*:\s*"?([^",}\r\n]+)'
    ) {
    ...
    $stellarTenantName = Get-StellarLabeledValue `
        -Text $stellarText `
        -Labels @("Tenant Name", "Tenant")
    ...
    # Normalize "SSOC: city" to the form's "SSOC-city" tenant value.
    ...
    $stellarSourceHost = Get-StellarLabeledValue `
        -Text $stellarText `
        -Labels @("Source Host", "Source IP", "SIP")

    if (-not $stellarSourceHost -and $stellarCaseScoreBreakdown) {
    ...
    $stellarDestinationHost = Get-StellarLabeledValue `
        -Text $stellarText `
        -Labels @("Destination Host", "Destination IP", "DIP")
    ...
    # Keep Description as one complete value so the alert paragraph and the
    # analyst template arrive in the same long-answer field.
    ...
    if (-not $stellarKillChainStage) {
        $stellarKillChainStage = Get-StellarLabeledValue `
            -Text $stellarText `
            -Labels @("Kill Chain Stage")
    }

    if (-not $stellarMitreTactic) {
        $stellarMitreTactic = Get-StellarLabeledValue `
            -Text $stellarText `
            -Labels @("MITRE Technique", "MITRE Tactic")
    }

    switch -Regex ($stellarMitreTactic) {
        '(?i)^Command and Control$'    { $stellarMitreTactic = "Command & Control"; break }
        '(?i)^Persistence$'            { $stellarMitreTactic = "Persistance"; break }
        '(?i)^Privilege Escalation$'   { $stellarMitreTactic = "Privilege Escaltion"; break }
    }
    ...
    # This classification requires the analyst's judgment in Google Forms.
    $stellarAnalystDetermination = "User input"
    ...
    # ==========================================
    # BUILD CLICK-TO-COPY FORM DATA
    # ==========================================

    $stellarSelectedAnalyst = [string]$stellarAnalystComboBox.SelectedItem

    $formCopyValues.Clear()

    # Dropdown fields: show the answer beside the field, but do not copy it.
    $formCopyValues["Tenant Name*"] = $stellarTenantName
    $formCopyValues["Analyst*"] = $stellarSelectedAnalyst
    $formCopyValues["Stellar Severity*"] = $stellarSeverity
    $formCopyValues["Kill Chain Stage*"] = $stellarKillChainStage
    $formCopyValues["MITRE Technique*"] = $stellarMitreTactic
    $formCopyValues["Analyst Determination*"] = $stellarAnalystDetermination
    $formCopyValues["Detection Type*"] = $stellarDetectionType
    $formCopyValues["Escalation Required*"] = "Mentor Review"

    # Text fields: clicking the row copies the complete value.
    $formCopyValues["Stellar #*"] = $stellarStellar
    $formCopyValues["Issue*"] = $stellarIssue
    $formCopyValues["URL*"] = $stellarUrlValue
    $formCopyValues["Who*"] = $stellarWho
    $formCopyValues["What*"] = $stellarWhat
    $formCopyValues["When*"] = $stellarWhen
    $formCopyValues["Where*"] = $stellarWhere
    $formCopyValues["Source Host*"] = $stellarSourceHost
    $formCopyValues["Destination Host*"] = $stellarDestinationHost
    $formCopyValues["JSON of the event*"] = $stellarJsonText
    $formCopyValues["Description*"] = $stellarDescriptionText
    $formCopyValues["Supporting Notes*"] = $stellarSupportingNotesText

    $formCopyGrid.Rows.Clear()
    foreach ($formCopyFieldName in $formCopyValues.Keys) {
        $formCopyFieldValue = [string]$formCopyValues[$formCopyFieldName]
        $formCopyDisplayedValue = $formCopyFieldValue

        if ([string]::IsNullOrWhiteSpace($formCopyDisplayedValue)) {
            $formCopyDisplayedValue = "(not detected)"
        }
        elseif ($formCopyDisplayedValue.Length -gt 120) {
            $formCopyDisplayedValue = $formCopyDisplayedValue.Substring(0, 117) + "..."
        }

        $formCopyActionText = "Click to copy"
        if ($formCopyDropdownFields -contains $formCopyFieldName) {
            $formCopyActionText = "Dropdown"
        }

        $formCopyRowIndex = $formCopyGrid.Rows.Add()
        $formCopyGrid.Rows[$formCopyRowIndex].Cells["FormField"].Value = $formCopyFieldName
        $formCopyGrid.Rows[$formCopyRowIndex].Cells["Answer"].Value = $formCopyDisplayedValue
        $formCopyGrid.Rows[$formCopyRowIndex].Cells["Action"].Value = $formCopyActionText
    }

    $formCopyGrid.ClearSelection()
    $formCopyPreviewBox.Clear()
    $formCopyStatusLabel.Text = "Ready. Dropdown answers are shown; click a text-field row to copy."
    $tabControl.SelectedTab = $formCopyTab
    $formCopyGrid.Focus()
...
$stellarTextBox.Add_KeyDown({
    ...
    # Enter prepares the click-to-copy form fields
...
$tabControl.Add_SelectedIndexChanged({
    ...
    elseif ($tabControl.SelectedTab -eq $formCopyTab) {
